...
class Tokenizer {
        ...
        while (this.column < FileContent.length) {
            switch (FileContent[this.column]) {
                ...
                case "\'":
                    ...
                    while (true) {
                        this.nextColumn();
                        single_quotes_content = single_quotes_content + FileContent[this.column]; //send to expression
                ...
                case "$":
                    ...
                    while (true) {
                        ...
                        if (FileContent[this.column] === " " ||
                            ...
                            break;
                        } else if (FileContent[this.column].match(/[$&+,:;=?@#|'<>.^*()%!-]/)) {
                            log(chalk.red(`* Can't use this special chars in variables. At line ${this.line}`));
                            process.exit();
                        }
                    }
    ...
    getAttribute(FileContent: string) {
        if (this.char_buffer.length > 0) {
            ...
            if (!Array.isArray(this.syntaxExpression[this.syntaxExpression.length - 1].attr)) {
                this.syntaxExpression[this.syntaxExpression.length - 1].attr = [];
            }
            this.syntaxExpression[this.syntaxExpression.length - 1].attr?.push(this.char_buffer + "=" + value);
            this.char_buffer = "";
        }
    }
    ...
    variablesExist(variable: string | number) {
        ...
        let variable_file = JSON.parse(this.fileManager.readFile(process.cwd() + "/src/var.json"));
