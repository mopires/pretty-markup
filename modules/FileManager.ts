...
class FileManager {
    ...
    GetFiles(subfolder: string = process.cwd() + "/src/", prettyMarkupFiles: Array < object > = []) {
        let folders;
        let path = subfolder;
    ...
    private WriteHTML(build: Build, File: any) {
        ...
        fs.writeFileSync(File.path.replace("src", "public") + this.swipeExtension(File.name), build.htmlCompiled, (e: any) => {
            if (e) throw e;
        });
